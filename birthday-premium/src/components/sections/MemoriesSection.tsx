...
function CoverFlowCarousel({ images, onSelect }: { images: string[]; onSelect: (src: string) => void }) {
  ...
  const cw = isDesktop ? 260 : 200
  const ch = isDesktop ? 325 : 250
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-20">
        {images.map((_, i) => (
          <button
            ...
            style={{
              width: i === ((current % images.length) + images.length) % images.length ? 18 : 4,
              height: 4,
              background: i === ((current % images.length) + images.length) % images.length
                ? 'rgba(244,63,94,0.7)'
